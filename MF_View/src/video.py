...
class Webcam:
    ...
    def update_snapshot_state(self, face=None):
        """
        Update the snapshot state based on the current preview state.
        
        Args:
            face: Optional face identifier (U, R, F, D, L, B) to associate with this scan.
        """
        self.snapshot_state = list(self.preview_state)
        
        # 获取中心块的颜色名称（用于传统的颜色识别方式）
        ...
        # 如果指定了面，则使用指定的面作为键，否则使用颜色名称
        if face and face in face_to_color:
            # 检查该面是否已经被扫描过
            color_name = face_to_color[face]
            if color_name in self.result_state:
                # 如果已经扫描过，则更新它
                print(f"更新面 {face} ({color_name})")
            else:
                print(f"录入面 {face} ({color_name})")
            
            self.result_state[color_name] = self.snapshot_state
        else:
            # 兼容原有行为，使用颜色名称作为键
        ...
        while True:
            ...
            if not self.calibrate_mode:
                ...
                if key == ord('U') or key == ord('u'):
                    self.current_selected_face = 'U'
                elif key == ord('R') or key == ord('r'):
                    self.current_selected_face = 'R'
                elif key == ord('F') or key == ord('f'):
                    self.current_selected_face = 'F'
                elif key == ord('D') or key == ord('d'):
                    self.current_selected_face = 'D'
                elif key == ord('L') or key == ord('l'):
                    self.current_selected_face = 'L'
                elif key == ord('B') or key == ord('b'):
                    self.current_selected_face = 'B'

                # Update the snapshot when space bar is pressed.
                if key == 32 and hasattr(self, 'current_selected_face') and self.current_selected_face:
                    self.update_snapshot_state(self.current_selected_face)
